...
export default function BookingWidget({ place }) {
  ...
  const [redirect, setRedirect] = useState('');
  const [error, setError] = useState('');
  const [loginRequired, setLoginRequired] = useState(false); // State variable to track if login is required
  const {user} = useContext(UserContext);
  ...
  async function reserve() {
    if (!user) {
      // If user is not logged in, set loginRequired to true and return
  ...
  return (
    <div className="bg-white shadow p-2 rounded-2xl">
      {loginRequired && <div className="text-red-600 p-2">Please login to make a reservation.</div>}
      <div className="text-2xl text-center">
        Price : ₹{place.price}
        <span className="text-100px font-thin"> night</span>
      </div>
